...
Listado.prototype.buscarRestaurante = function(id) {
    ...
    let finded = this.restaurantes.find(e => e.id === id)
    if (finded !== undefined)
    return finded
...
/* -------------------------------------------------------------------------- */
/*      función que elimina los elementos repetidos de un arreglo. creada     */
/* -------------------------------------------------------------------------- */

Listado.prototype.eliminaElemRepetidos = function(arrays) {
    let elemFiltrados = arrays.filter((elem, index, self) => index === self.indexOf(elem))
    return elemFiltrados.sort()
}
...
Listado.prototype.obtenerCiudades = function() {
    ...
    c = this.restaurantes.map(x => x.ubicacion)
    ...
    return c2.sort(); */
    return this.eliminaElemRepetidos(c)
}
...
Listado.prototype.obtenerRubros = function() {
    ...
    r = this.restaurantes.map(x => x.rubro)
    ...
    return r2.sort(); */
    return this.eliminaElemRepetidos(r)
}
...
Listado.prototype.obtenerHorarios = function() {
    ...
    arregloH = this.restaurantes.map(x => x.horarios)
    ...
    return h2.sort(); */
    return this.eliminaElemRepetidos(h).sort()
}
